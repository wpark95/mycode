...
def main():
    ...
    with open(f"{setcode.upper()}_cards.set", "w") as mtgfile:
        
        # print a line for each card
        for card in cards:
            print(file = mtgfile)
            
            # print card information in a more human-friendly format
            for key in card:
                print(f"{key}: {card.get(key)}", file = mtgfile)
